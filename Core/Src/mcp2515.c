...
/**
 * @file mcp2515.c
 * @brief MCP2515 CAN controller driver (SPI) for STM32L4.
 *
 * Provides basic SPI helpers, register read/write, configuration and
 * message transmit/receive helpers for the Microchip MCP2515 CAN controller.
 *
 * This module is a lightweight port of the commonly-used Arduino/embedded
 * MCP2515 drivers adapted for STM32 HAL SPI.
 */
...
/**
 * @brief Assert MCP2515 chip-select and start SPI transaction.
 */
void startSPI() {
...
/**
 * @brief De-assert MCP2515 chip-select and end SPI transaction.
 */
void endSPI() {
...
/**
 * @brief Transfer one byte over SPI to the MCP2515.
 *
 * Performs a full-duplex transmit/receive of a single byte using the
 * configured HAL SPI handle.
 *
 * @param txByte Byte to send.
 * @return Received byte from the MCP2515.
 */
...
/**
 * @brief Write multiple sequential registers starting at @p reg.
 *
 * The MCP2515 supports auto-incrementing register addresses, so this
 * helper writes @p n bytes starting at @p reg.
 *
 * @param reg Starting register address.
 * @param values Pointer to data to write.
...
/**
 * @brief Load TX buffer with identifier, control and data bytes.
 *
 * This helper assumes @p reg already contains the correct "load" command
 * byte (eg. LOAD_TX_BUFFER instruction variants).
 */
...
/**
 * @brief Atomically modify register bits using MCP2515 BIT MODIFY.
 *
 * @param reg Register address to modify.
 * @param mask Bit mask indicating which bits to change.
 * @param data New bit values (masked by @p mask).
 */
...
/**
 * @brief Read multiple sequential registers from the MCP2515.
 *
 * Reads @p n bytes starting at register @p reg into @p values.
 */
...
/**
 * @brief Read from RX buffer (command-coded) into a buffer.
 *
 * @param reg Read-command or address for the RX buffer operation.
...
CAN_Error MCP_reset(void)
{
    ...
    // Use this instead of HAL_Delay since reset is called before 
    // osKernelStart, which means the HAL tick increment function
