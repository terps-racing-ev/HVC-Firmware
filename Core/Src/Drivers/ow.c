...
void ow_init(ow_t *handle, const ow_init_t *init)
{
  ...
  /* Register user timer callback for timing events (optional depending on HAL config)
   *
   * When USE_HAL_TIM_REGISTER_CALLBACKS == 0, the application must call
   * ow_callback() from the selected timer IRQ handler.
   */
